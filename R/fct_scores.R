...
read_scores <- function() {
  ...
  scores_2021 <- readr::read_csv(fs::path(data_path, "score_data/scores_2021.csv"))
  scores_2022 <- readr::read_csv(fs::path(data_path, "score_data/scores_2022.csv"))

  scores_2020_matched <- scores_2020 |>
    dplyr::left_join(design_2020 |>
                       dplyr::distinct(pot, type, genotype_id)) |>
    dplyr::mutate(subset = "2020 Main",
                  treatment = "Infected")
  ...
  scores_2021_fungicide_matched <- scores_2021 |>
    dplyr::left_join(design_2021_fungicide |>
                       dplyr::mutate(pot = as.character(pot)) |>
  ...
  scores_2022_fungicide_matched <- scores_2022 |>
    dplyr::left_join(design_2022_fungicide |>
                       dplyr::mutate(pot = as.character(pot)) |>
                       dplyr::distinct(pot, type, genotype_id, treatment))|>
    dplyr::mutate(subset = "2022 Subset") |>
    dplyr::filter(!is.na(type))

  scores <- dplyr::bind_rows(scores_2020_matched, scores_2021_fungicide_matched,
                             scores_2021_Main_matched, scores_2022_fungicide_matched,
...
read_repeated_scores <- function() {
  data_path <- fs::path_package("abscreenmethods", "data-raw", "score_data", "rescoring_2021.csv")
...
get_scoring_repeatability <- function() {
  ...
  class_matrix <- repeated_scores |>
    dplyr::mutate(scoring_group = paste0("scorer_", scorer, "_round_", round)) |>
    tidyr::pivot_wider(id_cols = c(pot), names_from = scoring_group, values_from = score_group)
  ...
  a1_a2 <- yardstick::metrics(data = class_matrix, truth = scorer_A_round_1, estimate= scorer_A_round_2) |>
    fmt_yardstick_metrics("Within", "Scorer A, Round 1", "Scorer A, Round 2")
  b1_b2 <- yardstick::metrics(data = class_matrix, truth = scorer_B_round_1, estimate= scorer_B_round_2) |>
    fmt_yardstick_metrics("Within", "Scorer B, Round 1", "Scorer B, Round 2")

  # Between Scorer

  a1_b1 <- yardstick::metrics(data = class_matrix, truth = scorer_A_round_1, estimate= scorer_B_round_1) |>
    fmt_yardstick_metrics("Between", "Scorer A, Round 1", "Scorer B, Round 1")
  a1_b2 <- yardstick::metrics(data = class_matrix, truth = scorer_A_round_1, estimate= scorer_B_round_2) |>
    fmt_yardstick_metrics("Between", "Scorer A, Round 1", "Scorer B, Round 2")
  a2_b1 <- yardstick::metrics(data = class_matrix, truth = scorer_A_round_2, estimate= scorer_B_round_1) |>
    fmt_yardstick_metrics("Between", "Scorer A, Round 2", "Scorer B, Round 1")
  a2_b2 <- yardstick::metrics(data = class_matrix, truth = scorer_A_round_2, estimate= scorer_B_round_2) |>
