...
make_score_table <- function(scores = scores) {
  ...
  control_genotype_averages |>
    ...
    dplyr::rename("Genotype" = genotype_id, "Treatment" = treatment) |>
    reactable::reactable()
}
...
make_score_histogram <- function(
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::scale_color_manual(
      values = c(
        "Fungicide-treated" = color_palette_infection[1],
        "Infected" = color_palette_infection[2]
      )
    ) +
...
make_control_genotypes_score_dotplot <- function(scores = scores) {
  ...
  p <- scores |>
    ...
    dplyr::mutate(
      grouping_var = stringr::str_c(
        subset,
        ": ",
        substr(.data$treatment, 1, 3),
        "."
      ),
      grouping_var = factor(
        grouping_var,
    ...
    ggplot2::labs(
      x = "Disease Index",
      subtitle = "Control genotypes: DI per pot"
    ) +
    ggplot2::theme_bw() +
    ggplot2::theme(
      legend.title = ggplot2::element_blank(),
...
save_score_patchwork_plot <- function() {
  ...
  control_genotypes_score_dotplot <- make_control_genotypes_score_dotplot(
    scores
  )

  score_histograms <- subsets |>
    purrr::map(\(s) {
      p <- make_score_histogram(
        scores,
        s,
        add_fill_legend_when_only_infected = FALSE
      )
      print(p)
      p
    })
  ...
  ggplot2::ggsave(
    filename = "out/scores_patchwork.pdf",
    plot = p,
    units = "cm",
    width = 12,
    height = 10,
  ...
  valid_pots <- subsets |>
    purrr::map(\(x) {
      ...
      if (!is_main || (is_main && trait %in% c("n_lesions", "canon_fgcc"))) {
        removed <- remove_single_timepoint_outliers(
          tp_obj,
          trait = trait,
          subset = x
        )
      ...
      purrr::imap_chr(trait, \(values, subset) {
        ...
        result <- if (!is.na(mean_kept)) {
          paste0(
            round(mean_kept),
            " (",
            round(mean_kept_percentage, digits = 2),
            " %)"
          )
        } else {
          NA_character_
...
make_pot_single_timepoint_removal_tables <- function(counts) {
  purrr::imap(counts, \(valid, subset_name) {
    ...
    data <- data.frame(valid) |>
      ...
      dplyr::left_join(
        traits |> dplyr::select(date, dai, das) |> dplyr::distinct(),
        by = "date"
      )
