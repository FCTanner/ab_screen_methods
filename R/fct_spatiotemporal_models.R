...
fit_model <- function(tp_obj, trait, subset) {
  ...
  spatial_model <- statgenHTP::fitModels(
    tp_obj_dates_removed,
    trait = trait,
    geno.decomp = geno.decomp
  )
  corrected_values <- statgenHTP::getCorrected(spatial_model)
  spline <- statgenHTP::fitSpline(
    corrected_values,
    trait = trait,
    knots = knots
  )
...
run_spatiotemporal_modelling <- function(test_run = FALSE) {
  confirm_continue <- \() {
    cli::cli_alert(
      "Running the spatiotemporal models is a costly operation (~60min)"
    )
    repeat {
  ...
  out_subsets <- subsets |>
    ...
    purrr::map(\(s) {
      logger::log_info(
        "Run models for subset {s} ({counter_subset} / {n_subsets} subsets)"
      )
      ...
      out_traits <- traits |>
        ...
        purrr::map(\(t) {
          logger::log_info(
            "Model trait {t} ({counter_trait} / {n_traits} traits)"
          )
          ...
          daily_values <- predicted_values |>
            ...
            dplyr::summarise(
              mean_pred = mean(.data$pred.value, na.rm = TRUE)
            ) |>
            dplyr::ungroup()
